#!/usr/bin/env bash
# ==============================================================================
# RexOne Pre-Commit Secret Scanner
...
echo -e "${CYAN}🔍 Scanning for uncommitted secrets and sensitive files...${NC}"
...
# Get diff content (or all tracked content)
if [ "$SCAN_ALL" = true ]; then
  ...
  # Check staged diff additions only

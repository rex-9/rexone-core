...
- **Referral Generation**:
  - On every successful user signup, `User` model automatically creates a referral coupon (`REF<USERNAME>`, uppercase alphanumeric only, minimum 6 characters padded, 20% discount, `max_usage_per_user: 1`, `referrer_id: user.id`).

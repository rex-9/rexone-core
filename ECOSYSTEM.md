...
- **Media Compression Queue (`media`)**: Dedicated `media` worker process running `Media::CompressImageJob` (libvips), `Media::CompressVideoJob` (FFmpeg), and `Media::CompressAudioJob` (FFmpeg AAC / libmp3lame). WAV, FLAC, and OGG remux to M4A. SVG is converted to PNG at upload/save in the API process (not in the media worker); the stored asset is `png` and `optimal` (no auto image-compress pass, and admin compress is rejected as already optimal). `.srt` subtitle children are stored as `ready` on the API process and never enter this queue. Uses an **optimal-first pipeline**: if reduction is negligible (`< 3%`) or size doesn't decrease on initial upload, the asset is immediately marked as `optimal` without touching cache. If meaningful reduction is achieved, cache counter tracks passes with a fallback safety cap of 2 passes (`MAX_COMPRESSION_PASSES = 2`). Broadcasts real-time updates over ActionCable (`NotificationChannel`). Supports uploads up to 10 MB for images/non-videos and 100 MB for videos.

...
RSpec.describe "V1 Access API", type: :request do
  ...
  before do
    ...
    grant_permissions(user, "access", :read, :delete)
  end

  describe "GET /v1/access" do
    it "returns paginated user accesses" do
      ...
      get "/v1/access", params: { limit: 2 }, headers: headers
  ...
  describe "GET /v1/access/active" do
    it "returns only active accesses" do
      ...
      get "/v1/access/active", headers: headers
  ...
  describe "GET /v1/access/check" do
    it "checks if user has active access for a product" do
      ...
      get "/v1/access/check", params: { product_id: product.id }, headers: headers
  ...
  describe "DELETE /v1/access/:id" do
    let!(:access) { create(:access, user: user, product: product, status: AccessConstants::AccessStatus::ACTIVE) }

    it "revokes the user's access" do
      delete "/v1/access/#{access.id}", headers: headers
    ...
    it "rejects revoking access belonging to another user" do
      ...
      delete "/v1/access/#{other_access.id}", headers: headers

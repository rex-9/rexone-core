# app/jobs/media/compress_image_job.rb
...
module Media
  class CompressImageJob < ApplicationJob
    ...
    def perform(asset_id:)
      ...
      compressed_path = MediaService::ImageCompressor.compress(input_path)
      compressed_bytes = File.size(compressed_path)

      if compressed_bytes >= original_bytes
        # Compressed file is not smaller — original file is already optimal!
        Rails.logger.info("[CompressImageJob] Original (#{original_bytes} bytes) already optimal (compressed: #{compressed_bytes} bytes). Marking optimal immediately.")
        @asset.mark_optimal!
      ...
      Rails.logger.info("[CompressImageJob] Compressed #{original_bytes} -> #{compressed_bytes} bytes for asset #{asset_id} (#{(reduction_ratio * 100).round(1)}% reduction)")
      ...
      if reduction_ratio < MediaConstants::MIN_REDUCTION_THRESHOLD
        Rails.logger.info("[CompressImageJob] Asset #{asset_id} reduction (#{(reduction_ratio * 100).round(1)}%) below threshold. Marking optimal immediately.")
        @asset.mark_optimal!
        broadcast_status_change(MediaConstants::Status::OPTIMAL)
        return
      end
    ...
    def reupload_compressed(compressed_path)
      result = StorageService::Client.upload(
        ...
        folder: File.dirname(@asset.storage_key.to_s).presence || "admin_uploads/image",
        resource_type: "image",
        overwrite: true
      )
